...
void saveTxtMmap(string &outputFile){
    ...
    temp[idx]='\0';
    int totalSize=0;    //记录路径的大小
    for(int i=MIN_LEN;i<=MAX_LEN;++i) {
        totalSize += (addrOffset[i-MIN_LEN]-addrStart[i-MIN_LEN]);
    }
    totalSize+=idx;
    int fd = open(outputFile.c_str(),O_CREAT|O_RDWR|O_TRUNC,S_IRUSR|S_IWUSR);//必须设置读写权限,若只有写权限,会产生SIGSEGV信号
                                                            //mmap进行文件映射时必须先读取文件`
    int len=lseek(fd,totalSize-1,SEEK_CUR);  //从当前位置开始偏移 totalSize-1个字符
    int n=write(fd,"",1);
